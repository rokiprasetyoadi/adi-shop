...
class CartController extends Controller
{
    ...
    public function add_to_cart(Product $product, Request $request, Cart $cart)
    {
        ...
        $cek_cart = Cart::where('product_id', $product_id)->where('user_id', $user_id)->first();
        ...
        if($cek_cart == null){
            ...
            $request->validate([
                'amount' => 'required|gte:1|lte:'. ($product->stock - $cek_cart->amount)
            ]);

            $cek_cart->update([
                'amount' => $cek_cart->amount + $request->amount
            ]);
        }
